...
def publish_product(photo_bytes: bytes, caption: str, product_id: str) -> list[dict]:
    """Надсилає фото з підписом і кнопками в усі цілі (група + канал).

    Повертає список відповідей Telegram API (по одній на кожну ціль).
    """
    ...
    keyboard = __import__("json").dumps(build_keyboard(product_id))
    results = []
