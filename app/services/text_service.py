...
_SYSTEM = (
    ...
    "НЕ додавай ціну до підпису (майстриня вкаже ціну сама). "
...
def generate_caption(description: str, price: str | None = None, photo_bytes: bytes | None = None) -> str:
    ...
    if not config.ANTHROPIC_API_KEY:
        return description.strip() or "Ніжна квітка ручної роботи 💐"

    client = Anthropic(api_key=config.ANTHROPIC_API_KEY)
    hint = (f"Побажання майстрині до опису: {description}\n" if description.strip()
            else "Опису немає — склади сам за фото.\n")
